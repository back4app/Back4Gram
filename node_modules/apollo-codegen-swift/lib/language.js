...
class SwiftGenerator extends CodeGenerator_1.default {
    ...
    commentWithoutTrimming(comment) {
        comment &&
            comment.split("\n").forEach(line => {
    ...
    classDeclaration({ className, modifiers, superClass, adoptedProtocols = [] }, closure) {
        this.printNewlineIfNeeded();
        this.printOnNewline(printing_1.wrap("", printing_1.join(modifiers, " "), " ") +
    ...
    propertyDeclaration({ propertyName, typeName, description }) {
        this.comment(description);
        this.printOnNewline(`public var ${escapeIdentifierIfNeeded(propertyName)}: ${escapeIdentifierIfNeeded(typeName)}`);
    }
